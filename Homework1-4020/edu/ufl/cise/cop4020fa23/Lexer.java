...
public class Lexer implements ILexer {
	...
	private Kind mapper(String s) {
		// For reserved words
		if (reservedWords.contains(s.toLowerCase())) {
			return Kind.valueOf("RES_" + s.toLowerCase());
		}
		// For constant words
		else if (constantWords.contains(s.toUpperCase())) {
			return Kind.CONST;
		}
	...
	//			System.out.println("Meow" + counter);
		...
		return new Token(kind, 0, sb.length(), charList, new SourceLocation(line, column));
	}
